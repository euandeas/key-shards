...
<div class="mt-12">
	{#each resultasb64 as result, i}
		<div class="mb-6">
			<Input value="{result}" disabled class="select-text disabled:cursor-text disabled:opacity-100 mb-2"/>
			<div class="flex w-full">
				<Button id="bipbutton{i}" class="w-full py-2 px-4 rounded-e-none" on:click={() => toggleBIP(i)}>BIP</Button>
				<Button id="filebutton{i}" class="w-full py-2 px-4 border-x border-grey rounded-none" on:click={() => exportasfile(i)}>File</Button>
				<Button id="qrbutton{i}" class="w-full py-2 px-4 rounded-s-none" on:click={() => exportasqr(i)}>QR</Button>
			  </div>
		</div>
	{/each}
</div>
