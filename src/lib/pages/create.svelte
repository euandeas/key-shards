...
<div class="relative h-full overflow-y-hidden">
	<div class="z-0 h-full">
		...
		<Button class="mb-8 w-full" on:click={() => (showNewPage = !showNewPage)}>Generate</Button>
	</div>
	<div
		class={`z-1 absolute transition-all duration-1000 ease-in-out ${showNewPage ? 'inset-0' : 'left-0 right-0 top-full'}`}
		style="
